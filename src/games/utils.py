...
def sample_action(model, tokenizer, prompt, legal_moves: list[str], game_count) -> tuple[str, float]:
    """
    ...
    for move in legal_moves:
        move_ids = tokenizer(move, add_special_tokens=False, return_tensors="pt").input_ids[0].to(device)
    ...
    temperature = max(0.7, 1.5 - (game_count / 500))
